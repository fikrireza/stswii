...
class PartnerProductController extends Controller
{
		...
		public function index(Request $request)
		{
				...
				return view('partner-product.index', compact('request','provider','partner'));
		}
		...
		public function create()
		{
			...
			return view('partner-product.tambah', compact('getPartner','getProvider'));
		}
		...
		public function edit($id, $version)
		{
				...
				if ($retrun == 'index') {
					...
					return view('partner-product.ubah', compact('getPartnerProduct','getPartner','getProvider','getProduct'));
				}
		}
		...
		public function yajraGetData(Request $request)
		{
				...
				if ($f_active != null) {
					$getDatas->where('sw_partner_product.active', $f_active);
				}
				...
				$Datatables = Datatables::of($getDatas)
					...
					->addColumn('action', function ($getData) {
						...
						if (Auth::user()->can('update-partner-product')) {
							$actionHtml = $actionHtml."<a href='" . route('partner-product.edit', ['id' => $getData->partner_product_id, 'version' => $getData->version]) . "'' class='btn btn-xs btn-warning btn-sm' data-toggle='tooltip' data-placement='top' title='Update'><i class='fa fa-pencil'></i></a>";
						}
						if (Auth::user()->can('activate-product-sell-price')) {
							if ($getData->active == "Y") {
								$actionHtml = $actionHtml."<a href='' class='unpublish' data-value='" . $getData->partner_product_id . "' data-version='" . $getData->version . "' data-toggle='modal' data-target='.modal-nonactive'> <span class='btn btn-dark btn-xs' data-toggle='tooltip' data-placement='top' title='Non Active'><i class='fa fa-times'></i></span></a>";
							}
							else
							{
								$actionHtml = $actionHtml."<a href='' class='publish' data-value='" . $getData->partner_product_id . "' data-version='" . $getData->version . "' data-toggle='modal' data-target='.modal-active'> <span class='btn btn-success btn-xs' data-toggle='tooltip'	data-placement='top' title='Active'><i class='fa fa-check'></i></span></a>";
							}
						}
						if (Auth::user()->can('delete-partner-product')) {
							$actionHtml = $actionHtml."<a href='' class='delete' data-value='" . $getData->partner_product_id . "'data-version='" . $getData->version . "' data-toggle='modal' data-target='.modal-delete'><span class='btn btn-xs btn-danger btn-sm' data-toggle='tooltip' data-placement='top' title='Delete'><i class='fa fa-trash'></i></span></a>";
						}
						return $actionHtml;
					});
