...
class ProviderPrefixController extends Controller{
    ...
    public function index()
    {
    		...
    		$getProviderPrefix = ProviderPrefix::select('provider_id','provider_prefix_id','prefix','version')->get();

    		return view('provider-prefix.index', compact('getProvider','getProviderPrefix'));
    }
    ...
    public function store(Request $request)
    {
    		...
    		$validator = Validator::make($request->all(), [
                                			...
                                			'prefix' => 'required|numeric|digits_between:1,18|
    		...
    		if($validator->fails())
        {
    			return redirect()->route('provider-prefix.index')
                           ...
                           ->withInput()
                           ->with('add-false', 'Something Errors');;
    		}
    ...
    public function update(Request $request)
    {
    		...
    		if($validator->fails())
        {
    			return redirect()->route('provider-prefix.index')
                          ->withErrors($validator)
                          ->withInput()
                          ->with('update-false', 'Something Errors');;
    		}
    ...
    public function yajraGetData()
    {
    	...
    	$getProviderPrefixs = ProviderPrefix::leftJoin('sw_provider', 'sw_provider.provider_id', 'sw_provider_prefix.provider_id')
	    	                                  ->select(['sw_provider_prefix.provider_id as provider_id','sw_provider.provider_code as provider_code','provider_prefix_id','prefix','sw_provider_prefix.version'])->get();
        ...
        return Datatables::of($getProviderPrefixs)
            ...
            ->addColumn('action', function ($getProviderPrefix) {
                ...
                {
        					$actionHtml = $actionHtml."<a href='' class='delete' data-value='".$getProviderPrefix->provider_prefix_id."'data-version='".$getProviderPrefix->version."' data-toggle='modal' data-target='.modal-delete'><span class='btn btn-xs btn-danger btn-sm' data-toggle='tooltip' data-placement='top' title='Delete'><i class='fa fa-trash'></i></span></a>";
        				}
              return $actionHtml;
              })
